...
| --- | --- | --- |
| **Header** | **Fixo e transparente sobre o hero** (`position: fixed`, sem cor própria): o gradiente do hero passa por trás dele. Ao rolar, o JS adiciona `.is-scrolled` e ele ganha fundo sólido + sombra. Abaixo de 1024px vira **menu hambúrguer**. | É como o Figma mostra: header e hero são um bloco só. O fundo sólido ao rolar mantém o menu legível sobre as seções claras. O `scroll-padding-top` faz as âncoras pararem logo abaixo dele. |
