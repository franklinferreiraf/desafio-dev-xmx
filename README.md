...
| **Why** | **Divisor do topo com os valores do Figma:** duas formas brancas de 1048,22 × 91,98 (raio 20px) em top -31, left -419 e left 1293,78 (girada 180°), com a ponta interna cortada em diagonal (medida no export). Na página, os 15px de cima do frame ficam sob a seção anterior; no mobile a aba ocupa a tela inteira e o topo fica reto, como no `Mobile.png`. Eyebrow acima do título. | O export mostra os dois. |
| **Why** | **Eyebrow "INGREDIENTS"** acima de "Why Thousands Choose Tenurima™". | **Inconsistência detectada no Figma e mantida por fidelidade:** a seção é de benefícios, mas o `Mobile.png` e o `Desktop.png` mostram "INGREDIENTS" (provavelmente copiado da seção anterior). Repliquei o texto do design; se for erro, a troca é uma palavra no `index.html`. |
| **Why choose** | **Abaixo de 768px: os 6 badges numa lista única**, com espaçamento uniforme, e só depois dela o produto no pedestal, centralizado. **Tablet (768–1023px): as duas listas lado a lado (esquerda / direita), produto embaixo.** Desktop: badges / produto / badges, com o produto posicionado a partir do bloco dos cards. | Antes, no tablet, cada lista virava um grid de 2 colunas e ficava 2 + 1 badges. Ancorado no topo da seção, o pote cobria o título em 1024px. |
| **Why (mobile)** | **Produto com os valores do frame mobile do Figma** (540px de largura, escalado para a tela até 540px): `produto.png` com 602px em left -31 / top 824, entrando 11,5px sob o último badge; a seção termina em 1445,5 (início do marquee), cortando a base do pedestal; duas `pill capsule.003` (72,89 × 69,70, 83,95°, blur 6px) em 353 / 1086 e 110 / 1259 — no Figma esses valores são a caixa já girada, e o PNG (`hero-capsule-1`) já vem girado. | Posições conferidas no `Mobile.png`: as cápsulas caem nos centros medidos (±4px) e o produto foi ajustado por correlação. Os blocos "produto" (489,49 × 571,48 em 25 / 945) e o `mockup-tenurima-1` (462 × 407,48) não têm arquivo correspondente com esse recorte; o `produto.png` mantém com o bloco "produto" a mesma relação do desktop (-57 / -121). |
| **Why (mobile)** | **Foto do casal atrás do produto**: 603 × 462 (0,61 do tamanho do desktop) em left -136 / top 885 do frame mobile, com a mesma máscara, opacidade (.3) e blend do desktop; ancorada na base da seção para acompanhar o pote. | É onde ela está no `Mobile.png` (posição e escala medidas por correlação; o brilho na região do rosto da senhora fica a 2–3 níveis do export). Antes, no mobile, ela usava a posição do desktop reduzida e aparecia no topo da seção. |
...
| **Testimonials** | **Exatamente 4 depoimentos** no array `TESTIMONIALS` (`script.js`), com as 4 fotos reais. **1 card por vez < 1024px e 2 por vez ≥ 1024px.** Avança **1 card por clique**, sem loop: no último card a seta "próximo" fica branca/desabilitada. Também tem teclado (← →) e swipe; **sem bolinhas de paginação**, que não existem no Figma. | O carrossel de fato passa pelos 4 (Michael+Susan → Susan+Walter → Walter+Diane). Sem loop, o usuário sabe quando viu todos. O número de cards por vez vem de `--per-view` no CSS, então o breakpoint fica num só lugar. |
...
| **Testimonials** | **Abaixo de 768px o próximo card aparece na borda direita da tela** (prévia): o card ativo ocupa a coluna inteira e o seguinte vem 20px depois, cortado pela borda. O passo do slide é definido no CSS (`--slide-step`, com o JS só passando o índice em `--index`), então a posição continua certa ao redimensionar. Setas nos dois estados do export: **esquerda branca com contorno claro e seta vermelha** (também quando desabilitada, no primeiro card), **direita vermelha sólida**. | É o que o `Mobile.png` mostra (card de 237px e o seguinte a ~21px). |
| **Testimonials** | A foto fica **colada na borda do card** (sem respiro) e, a partir de 1280px, as **setas saem para as laterais** — a da esquerda branca com seta vermelha, a da direita vermelha. | É o layout do Figma. Abaixo de 1280px não há espaço fora do container: em 1024px as setas passavam da viewport (peguei isso no teste de overflow). |
...
| `768px–1023px` | `.why__stage`, `.why__product` | Listas lado a lado, produto embaixo (abaixo de 768px, lista única). |
| `max-width: 767px` | `.carousel__viewport`, `.carousel__track`, `.carousel__slide` | Prévia do próximo depoimento na borda direita. |
